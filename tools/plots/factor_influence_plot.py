...
from tools.plots.base import map_colors
import tools.adata_interface.core as adi
import tools.adata_interface.regulators as ri
from tools.tf_targeting import _driver_TF_test
...
def _influence_plot(ax, l1_pvals, l2_pvals, factor_names, pval_threshold = (1e-5, 1e-5), label_factors = None,
    ...
    label_closeness = 2, max_label_repeats = 1, axlabels = ('list1', 'list2'), color = 'grey', fontsize = 12, size=20):
    ...
    ax.scatter(x, y, c = cell_colors, s=size)
    ...
    ax.axline((1, 1), slope=1, color='royalblue')
    ax.set(xlabel = axlabels[0], ylabel = axlabels[1])
    ax.spines["right"].set_visible(False)
    ax.spines["top"].set_visible(False)
    print("x pvalues: {}, yvalues :{}, labels:{}".format(x[name_mask],y[name_mask], np.array(factor_names)[name_mask]))
    #ax.axis('square')
    plt.tight_layout()
...
def plot_factor_influence(
    ...
    ax = None, axlabels = ('list1', 'list2'), pval_pseudocount = 1e-300, size=20
):
    ...
    return _influence_plot(ax, l1_pvals, l2_pvals, factor_names, pval_threshold = pval_threshold, 
        ...
        axlabels = axlabels, color = color, fontsize = fontsize, size=size)

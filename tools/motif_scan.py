...
import tqdm
import re
from .core import wraps_functional
from .regulators import fetch_peaks, add_factor_hits_data
from functools import partial

logger = logging.getLogger(__name__)

mira_dir =os.path.dirname(os.path.realpath(__file__))

PWM_DIR = os.path.join(mira_dir, 'motifs/' )
PWM_suffix = 'jaspar'
PWM_DIR_MOUSE =  os.path.join(mira_dir, 'motifs/mouse/' )
PWM_DIR_HUMAN =  os.path.join(mira_dir, 'motifs/human/' )
...
def get_motif_glob_str(species="mouse"):
    if species == "mouse":
        return os.path.join(PWM_DIR_MOUSE, '*.{}'.format(PWM_suffix))
    elif species =="human":
        return os.path.join(PWM_DIR_HUMAN, '*.{}'.format(PWM_suffix))
...
def get_motif_hits(peak_sequences_file, num_peaks, species="mouse", pvalue_threshold = 0.00005):
...
def get_motif_hits_in_peaks(peaks, pvalue_threshold = 0.0001,*, species="mouse", genome_fasta):
    '''
    ...
    try:
        ...
        # print("genome_fasta file {}".format(genome_fasta))

        hits_matrix = get_motif_hits(temp_fasta_name, len(peaks), species = species, pvalue_threshold = pvalue_threshold)

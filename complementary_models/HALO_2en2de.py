    ...
    def inference(
       ...
       # Z Encoders for independent (time-lagging)
        ...
        if n_samples > 1:
            ...
            untran_za_dep = qz_acc.sample((n_samples,))
            z_acc_dep = self.z_encoder_accessibility.z_transformation(untran_za_dep)
            untran_zr_dep = qz_expr.sample((n_samples,))
            z_expr_dep = self.z_encoder_expression.z_transformation(untran_zr_dep)
        ...
        ## get the decoupled part of 
     
        z_expr = Normal(qzm_expr, qzv_expr.sqrt()).rsample()
        z_expr_dep = z_expr[:, :self.n_latent_dep]
        z_expr_indep = z_expr[:, self.n_latent_dep:]


        z_acc = Normal(qzm_acc, qzv_acc.sqrt()).rsample()
        ...
        outputs = dict(
            ...
            ### pay attention of this part what is z for 
            z=z_expr,
            qz_m=qzm_expr,
            qz_v=qzv_expr
        )
...
class HALOVICAT3(MultiVI_Parallel):

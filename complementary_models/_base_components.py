...
class FCLayers(nn.Module):
    ...
    def forward(self, x: torch.Tensor, *cat_list: int):
        ...
        # print("self.n_cat_list {}, cat_list {}".format(self.n_cat_list, cat_list)) 
        if len(self.n_cat_list) > len(cat_list):
            raise ValueError(
                "nb. categorical args provided doesn't match init. params."
            )
        # print(self.n_cat_list, cat_list)    
...
class NeuralDecoderRNA(nn.Module):
    ...
    def __init__(
        ...
        super(NeuralDecoderRNA, self).__init__()
    ...
    # @torch.no_grad
    def _get_softmax_denom(self, z: torch.Tensor, *cat_list: int):
        individual_outputs = self.calc_feature_outputs(z, self.n_cat_list)
        z_feat = torch.cat(individual_outputs, dim=-1)
        raw_px_scale = self.factor_regressor(z_feat, self.n_cat_list)
        return raw_px_scale.exp().sum(-1)
